...
import com.domain.myapp.monitoring.ApplicationStatus;
...
public class CustomerController {
		...
		if (errors.size() == 0) {
			...
			try {
				response.sendError(SERVICE_UNAVAILABLE,
						"Self test failed with errors: {} " + errors);
			} catch (IOException e) {
	...
	public ApplicationStatus getApplicationStatus() {
		...
		return ApplicationStatusProjection
				.askApplicationStatus(applicationStatusProjection);
	}

	private void askEventStore(List<String> errors) {
		Future<Object> askEventStore = ask(eventStore,
				new AcknowledgePreviousEventsProcessed(), Timeout.THREE_SECONDS);
		try {
			Object result = Await.result(askEventStore,
					Duration.create(Timeout.THREE_SECONDS_STR));
