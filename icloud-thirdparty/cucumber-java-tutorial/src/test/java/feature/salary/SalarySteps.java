...
public class SalarySteps {
	...
	@Given("^the salary the following data$")
	public void the_salary_the_following_data(final List<Employee> employees)
			throws Throwable {
		manager = new SalaryManager(employees);
	}

	@When("^the boss  id '(\\d+)' by (\\d+)%$")
	public void the_boss_id_by(final int id, final int increaseInPercent)
			throws Throwable {
		manager.increaseSalary(id, increaseInPercent);
	}

	@Then("^the id '(\\d+)' of (\\d+)$")
	public void the_id_of(final int id, final float salary) throws Throwable {
		Employee nominee = manager.getPayroll(id);
		assertThat(nominee.getSalary(), equalTo(salary));
	}
}
